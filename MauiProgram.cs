...
public static class MauiProgram
{
    public static MauiApp CreateMauiApp()
    {
        ...
        if (useMockServices)
        {
            builder.Services.AddSingleton<IPaymentSubscriptionApiClient, MockPaymentSubscriptionApiClient>();
        }
        else
        {
            builder.Services.AddHttpClient<IPaymentSubscriptionApiClient, PaymentSubscriptionApiClient>("PaymentServiceClient", client =>
            {
                var paymentBaseAddress = configuration["PaymentServiceEndpoint"];
                if (string.IsNullOrEmpty(paymentBaseAddress))
                {
                    throw new ArgumentNullException(nameof(paymentBaseAddress), "Payment Service Endpoint configuration is missing or empty.");
                }

                client.BaseAddress = new Uri(paymentBaseAddress);
            })
                .AddHttpMessageHandler<AuthHeaderHandler>();
        }
